...
5. **Navigation flow & rationale.** The assignment asks not only for a flow but for its justification. Content: the records list is the root screen (the user's data is the app's home; the most frequent action is reviewing records); adding a record is a modal sheet (a short, self-contained create task — modality signals "finish or cancel", keeps the list visually underneath as context, and dismisses cleanly in both orientations); the sheet returns via an `onSaved` callback that triggers a list reload (explicit data-flow seam instead of shared mutable state). State that the flow follows Apple's Human Interface Guidelines, naming the specific patterns verified in the code: a sheet for a self-contained create task (HIG *Modality*); `Cancel`/`Save` in the sheet's `.cancellationAction`/`.confirmationAction` placements with `Save` disabled until the form is valid; interactive dismiss blocked only while a save is in flight; destructive batch delete behind a confirmation dialog (swipe-to-delete needs none — the gesture is its own confirmation); the standard Edit/Done list-editing pattern with a contextual bottom-bar action; a segmented control for the mutually exclusive filter views. Claim only these verified patterns — no blanket "fully HIG-compliant" wording. Mention the router/screen-factory infrastructure in one sentence and defer detail to ARCHITECTURE.md.
...
3. **Folder organization — where to put what.** An annotated folder tree of the `SportRecord` package showing the layer-to-directory mapping, so it is unambiguous where a new file belongs:
   - `Domain/Entities/` — value-type entities and typed error/result types;
   - `Domain/Repositories/` — repository protocols;
   - `Domain/UseCases/` — one file per use case (protocol + default implementation);
   - `Data/DataSources/Local/` — SwiftData model, mapping, local data source;
   - `Data/DataSources/Remote/` — DTO, mapping, Firestore data source;
   - `Data/Repositories/` — repository implementations;
   - `Presentation/<Feature>/View/` and `Presentation/<Feature>/ViewModel/` — one folder per screen (e.g. `List`, `AddRecord`);
   - `Presentation/Shared/` — presentation helpers shared across screens;
   - `Composition/` — package-level factory helpers the app target composes from.
   Plus the equivalent short map for `Core` (`DesignSystem/`, `Networking/`, `Logging/`) and the app target (`DI/`, `Navigation/`). Tests mirror the source layout (`Tests/SportRecordTests/{Domain,Data,Presentation,Support}`).
...
5. **Navigation.** `AppRouter` (`Route` push stack — currently uninhabited — and `Sheet` modals), `AppFlowView` owning the `NavigationStack`, `ScreenFactory` as the composition point building View + ViewModel pairs. Screens receive navigation as injected closures and never see the router; the feature package stays navigation-agnostic.
6. **Dependency injection.** FactoryKit registrations and scope choices (singletons for the model container, data sources, repository, network monitor; cached use cases; ViewModels never container-managed — fresh per screen, constructor-injected).
7. **Concurrency.** Swift 6 strict concurrency: `Sendable` domain values, `@ModelActor` isolating SwiftData's non-Sendable `ModelContext`, `@MainActor @Observable` ViewModels, typed throws for the delete failure mode.
8. **Testing.** Swift Testing (not XCTest); protocol fakes for data sources, use cases, and the network monitor; real in-memory `ModelContainer` for local data-source tests; ViewModel tests cover state transitions and the delete partial-failure matrix. How to run the suites (same verified commands as the README).
